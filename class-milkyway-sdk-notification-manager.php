<?php
/**
 * The notification manager class for milkyway SDK
 *
 * @package     milkywaySDK
...
if ( ! class_exists( 'milkyway_SDK_Notification_Manager' ) ) :
	/**
	 * Notification manager model for milkyway SDK.
	 */
	class milkyway_SDK_Notification_Manager {
		/**
		...
		/**
		 * @var milkyway_SDK_Product Current product.
		 */
		private $product;
		/**
		 * @var array milkyway_SDK_Feedback Feedbacks available.
		 */
		private $callbacks = array();

		/**
		 * milkyway_SDK_Notification_Manager constructor.
		 *
		...
		function setup_notifications() {
			...
			if ( ( time() - $this->product->get_install_time() ) > self::NOTIFICATION_INTERVAL_HOURS * HOUR_IN_SECONDS ) {
				if ( $this->product instanceof milkyway_SDK_Product && $this->callbacks && is_array( $this->callbacks ) ) {
		...
		function show_notification() {
			...
			$active    = get_option( 'milkyway_sdk_active_notification', array() );
			...
			foreach ( $available as $key ) {
				...
				if ( $instance->can_notify() ) {
					...
					if ( empty( $active['key'] ) || ( $active['key'] != $key ) ) {
						$active['key']  = $key;
						$active['time'] = time();
						update_option( 'milkyway_sdk_active_notification', $active );
					}
