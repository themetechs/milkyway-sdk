...
if ( ! class_exists( 'milkyway_SDK_Endpoints' ) ) :
	/**
	 * Expose endpoints for milkyway SDK.
	 */
	final class milkyway_SDK_Endpoints {

		const SDK_ENDPOINT         = 'milkyway-sdk';
		const SDK_ENDPOINT_VERSION = 1;

		const HASH_FILE = 'milkyway-hash.json';

		// if true, the endpoint will expect a product slug and will return the value only for that.
		const PRODUCT_SPECIFIC = false;

		/**
		 * @var milkyway_SDK_Product $products Array of milkyway Product.
		 */
		static protected $products = array();

		/**
		 * milkyway_SDK_Endpoints constructor.
		 *
		 ...
		 *
		 * @param milkyway_SDK_Product $product milkyway Product.
		...
		private function generate_diff( $product, $files, $recurse = false ) {
			...
			if ( ! $wp_filesystem->is_readable( $path . '/' . self::HASH_FILE ) ) {
				return new WP_Error( 'milkyway_sdk_hash_not_found', sprintf( '%s not found', self::HASH_FILE ) );
			}
		...
		private function validate_params( WP_REST_Request $data, $params ) {
			...
			foreach ( $params as $param ) {
				...
				if ( empty( $value ) ) {
					return new WP_Error(
						'milkyway_' . $param . '_invalid', sprintf( 'Invalid %', $param ), array(
							'status' => 403,
						)
					);
				} else {
