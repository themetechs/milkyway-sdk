<?php
/**
 * The blog dashboard model class for milkyway SDK
 *
 * @package     milkywaySDK
 * @subpackage  Widgets
...
if ( ! class_exists( 'milkyway_SDK_Widget_Dashboard_Blog' ) ) :
	/**
	 * Blog dashboard widget model for milkyway SDK.
	 */
	class milkyway_SDK_Widget_Dashboard_Blog extends milkyway_SDK_Widget {
		...
		/**
		 * milkyway_SDK_Widget_Dashboard_Blog constructor.
		 *
		 * @param milkyway_SDK_Product $product_object The product object.
		 */
		...
		public function setup_hooks_child() {
			...
			add_filter( 'milkyway_sdk_recommend_plugin_or_theme', array( &$this, 'recommend_plugin_or_theme' ) );
		}
		 ...
		 */
		function setup_vars() {
			$this->dashboard_name = apply_filters( 'milkyway_sdk_dashboard_widget_name', 'WordPress Guides/Tutorials' );
			$this->feeds          = apply_filters(
				'milkyway_sdk_dashboard_widget_feeds', array(
					'https://milkyway.com/blog/feed',
				)
			);
		}
		...
		function add_widget() {
			...
			if ( isset( $wp_meta_boxes['dashboard']['normal']['core']['milkyway'] ) ) {
				return;
			}
			wp_add_dashboard_widget(
				'milkyway', $this->dashboard_name, array(
					&$this,
		...
		private function setup_feeds() {
			...
			if ( false === ( $items_normalized = get_transient( 'milkyway_sdk_feed_items' ) ) ) {
				...
				set_transient( 'milkyway_sdk_feed_items', $items_normalized, 48 * HOUR_IN_SECONDS );
			}
		...
		function render_dashboard_widget() {
			...
			?>
			<style type="text/css">
				#milkyway ul li.ti-dw-recommend-item {
					padding-left: 7px;
			...
			<?php
			$recommend = apply_filters( 'milkyway_sdk_recommend_plugin_or_theme', array() );
			if ( is_array( $recommend ) && ! empty( $recommend ) ) {
				...
				if ( ( $type == 'theme' && current_user_can( 'install_themes' ) ) || ( $type == 'plugin' && current_user_can( 'install_plugins' ) ) ) {
					...
					<span class="ti-dw-recommend-item ">
							<span class="ti-dw-recommend"><?php echo apply_filters( 'milkyway_sdk_dashboard_popular_label', sprintf( 'Popular %s', ucwords( $type ) ) ); ?>
								: </span>
						<?php
						echo trim(
							str_replace(
						...
						(<a class="thickbox open-plugin-details-modal"
							href="<?php echo $url . '&TB_iframe=true&width=600&height=500'; ?>"><?php echo apply_filters( 'milkyway_sdk_dashboard_install_label', 'Install' ); ?></a>)
					</span>
		...
		function get_product_from_api() {
			if ( false === ( $products = get_transient( 'milkyway_sdk_products' ) ) ) {
				$products         = array();
				$milkyway_themes = $this->get_themes_from_wporg( 'milkyway' );
				$codeinwp_themes  = $this->get_themes_from_wporg( 'codeinwp' );

				$milkyway_plugins = $this->get_plugins_from_wporg( 'milkyway' );
				$codeinwp_plugins  = $this->get_plugins_from_wporg( 'codeinwp' );

				$all_themes = array_merge( $milkyway_themes, $codeinwp_themes );
				...
				$all_plugins = array_merge( $milkyway_plugins, $codeinwp_plugins );
				...
				set_transient( 'milkyway_sdk_products', $products, 6 * HOUR_IN_SECONDS );
			}
