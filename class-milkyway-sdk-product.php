<?php
/**
 * The product model class for milkyway SDK
 *
 * @package     milkywaySDK
 * @subpackage  Product
...
if ( ! class_exists( 'milkyway_SDK_Product' ) ) :
	/**
	 * Product model for milkyway SDK.
	 */
	class milkyway_SDK_Product {
		/**
		...
		/**
		 * milkyway_SDK_Product constructor.
		 *
		...
		public function setup_from_fileheaders() {
			...
			if ( $this->is_external_author() ) {
				$this->store_url  = 'https://milkyway.com';
				$this->store_name = 'milkyway';
			}
		...
		public function is_logger_active() {
			...
			if ( ! $this->is_wordpress_available() ) {
				...
				if ( ! empty( $pro_slug ) ) {
					...
					$all_products = milkyway_SDK_Loader::get_products();
